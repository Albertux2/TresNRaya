...
class TableroTest {
	...
	void trnrHor() {
		...
		assertTrue(tablero.isHorizontal(Tipo.X));
	}
	
	@Test
	void trnrVer() {
		int posX[] = {0,1,2,1,2,2};
		int posY[] = {0,0,0,1,2,1};
		Tipo tipos[]= {Tipo.X,Tipo.X,Tipo.X,Tipo.O,Tipo.O,Tipo.O};
		for (int i = 0; i < tipos.length; i++) {
			tablero.colocarFicha(new Coordenada(posX[i],posY[i]), tipos[i]);
		}
		assertTrue(tablero.isVertical(Tipo.X));
	}
	...
	void trnrDiagonal() {
		int posX[] = {0,1,2,1,2,2};
		int posY[] = {0,0,0,1,2,1};
		Tipo tipos[]= {Tipo.O,Tipo.X,Tipo.X,Tipo.O,Tipo.O,Tipo.X};
		for (int i = 0; i < tipos.length; i++) {
			tablero.colocarFicha(new Coordenada(posX[i],posY[i]), tipos[i]);
		}
		assertTrue(tablero.isDiagonal(Tipo.O));
	}
